 ...
 */
	static async create({ message, content, role, channelID }) {
		...
		for (const buttonObject of role.roles) {
			dropdownsOptions.push(new MessageMenuOption().setEmoji(buttonObject.emoji).setLabel(buttonObject.label).setValue(buttonObject.role).setDescription(`klik disini untuk mendapatkan role ${message.guild.roles.cache.get(buttonObject.role).name}!`));
		}

		const dropdown = new MessageMenu().addOptions(dropdownsOptions).setID('dr').setPlaceholder('Klik Disini	');
